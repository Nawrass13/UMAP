...
REM Run Docker build with simplified command (avoiding complex quote escaping)
docker run --rm -v "%cd%":/src -w /src ubuntu:22.04 bash -c "apt-get update && apt-get install -y build-essential cmake libstdc++-11-dev && mkdir -p build-linux && cd build-linux && cmake .. -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON -DBUILD_TESTS=ON -DCMAKE_POSITION_INDEPENDENT_CODE=ON && make -j4 && ls -la . && echo Library copying... && if [ -f libuwot.so ]; then cp libuwot.so libuwot_backup.so; fi && for f in libuwot.so.*; do if [ -f \$f ] && [ ! -L \$f ]; then cp \$f libuwot_final.so && echo Copied \$f to libuwot_final.so; fi; done && echo Running Linux validation tests... && LINUX_ALL_TESTS_PASSED=1 && echo Test 1: Standard comprehensive validation... && if [ -f ./test_standard_comprehensive ]; then ./test_standard_comprehensive && echo Test 1 PASSED || (echo Test 1 FAILED && LINUX_ALL_TESTS_PASSED=0); else echo test_standard_comprehensive not found && LINUX_ALL_TESTS_PASSED=0; fi && echo Test 2: Error fixes validation... && if [ -f ./test_error_fixes_simple ]; then ./test_error_fixes_simple && echo Test 2 PASSED || (echo Test 2 FAILED && LINUX_ALL_TESTS_PASSED=0); else echo test_error_fixes_simple not found && LINUX_ALL_TESTS_PASSED=0; fi && echo Test 3: Comprehensive pipeline validation... && if [ -f ./test_comprehensive_pipeline ]; then ./test_comprehensive_pipeline && echo Test 3 PASSED || (echo Test 3 FAILED && LINUX_ALL_TESTS_PASSED=0); else echo test_comprehensive_pipeline not found && LINUX_ALL_TESTS_PASSED=0; fi && echo Test 4: Enhanced wrapper validation... && if [ -f ./test_enhanced_wrapper ]; then ./test_enhanced_wrapper && echo Test 4 PASSED || (echo Test 4 FAILED && LINUX_ALL_TESTS_PASSED=0); else echo test_enhanced_wrapper not found && LINUX_ALL_TESTS_PASSED=0; fi && if [ \$LINUX_ALL_TESTS_PASSED -eq 1 ]; then echo ALL 4 Linux tests completed successfully; else echo Some Linux tests failed - build may have issues; fi"
