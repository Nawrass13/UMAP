...
bool test_progress_reporting() {
    ...
    printf("Testing uwot_fit_with_progress_with_progress...\n");
    int result = uwot_fit_with_progress_with_progress(model, data.data(), n_obs, n_dim,
...
bool test_basic_functionality() {
    ...
    int result = uwot_fit_with_progress(model, data.data(), n_obs, n_dim,
        ...
        UWOT_METRIC_EUCLIDEAN, embedding.data(), nullptr, 0, 16, 200, 64);
...
bool test_model_persistence() {
    ...
    int result = uwot_fit_with_progress(model, train_data.data(), n_obs, n_dim,
        embedding_dim, 15, 0.1f, 1.0f, 50,
